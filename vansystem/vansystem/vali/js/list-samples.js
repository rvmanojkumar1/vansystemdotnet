...
function triggerUpdate() {
	...
	$(document).on('click','#sampleDetails .edit-sample',function(){
		
		var id = $(this).data('id');
		var dataType = $('#sampleDetails').data('type');
...
$(document).ready(function () {
	...
	var dataType = $('#sampleDetails').data('type'); //console.log(dataType);

	table = $('#sampleDetails').DataTable({
		"bProcessing": true,
		"bServerSide": true,
		"ajax": {
			"url": "/Report/loadSamplesData",
			"type": "POST",
			"data": {
				"type" : dataType
			}
		...
		buttons: [
			...
			/*{
				text: '<i class="fa fa-file-excel-o fa-fw"></i> Download XL</h2>',
				action: function() {
					//var query = $('#formTable .dataTables_scrollHead :input').serialize();
					window.location = '/Report/downloadXls?type='+dataType; //+query;
				}
			}*/
		]
